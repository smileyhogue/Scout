...
var games = new Datastore({
  filename: './data/games-database.db',
  autoload: true
});
var players = new Datastore({
  filename: './data/players-database.db',
  autoload: true
});
...
app.get('/', function(request, response) {
  response.render('index', { title: 'Scout', page: 'Home' });
});

app.get('/join', function(request, response) {
  response.render('join', { title: 'Scout', page: 'Join a game' });
});
...
app.get('/create', function(request, response) {
  response.render('create', { title: 'Scout', page: 'create a game' });
});

app.post('/create-game', function(request, response) {
  request.on('data', function(args) {
    var parameters = Parameters.fromString(args);
    DatabaseTool.createEntryID(games, { active: false, first: -1 }, function(
      id
    ) {
      DatabaseTool.createEntryID(
        players,
    ...
    games.findOne({ id: gameID }, function(err, doc) {
      ...
      if (!active) {
        players.find({ game_id: gameID }, function(err, doc) {
          ...
          games.update(
            { id: gameID },
            { $set: { active: true, first: first } },
            {},
            function() {
              Games.randomize(games, locations, gameID, function(
                gameID,
                location,
                roles
              ) {
...
app.post('/restart-game', function(request, response) {
  request.on('data', function(args) {
    var parameters = Parameters.fromString(args);
    var gameID = parameters['game_id'];
    games.update({ id: gameID }, { id: gameID, active: false }, {}, function(
      err,
      doc
    ) {
      ...
      players.find({ game_id: gameID }, function(err, doc) {
        ...
        for (var x in doc) {
          ...
          players.update(
            { id: id },
            { id: id, game_id: gameID, name: name },
            {},
            function(err, doc) {
              completed++;
              if (completed >= total) {
